...
class RNN(AbstractRNN):
    """
    Class implementing RNN algo with tensorflow
    """
    def __init__(self, **kwargs):
        ...
        _sentinelle: put there to prevent positional parameters.
                     Do not touch.
        """
        if kwargs.pop('cell_type', 'RNN') != 'RNN':
            raise ValueError("cell_type must be 'RNN'")
        super(RNN, self).__init__(cell_type='RNN', **kwargs)

    def build_cell(self):
        cell = tf.nn.rnn_cell.BasicRNNCell(self.multi_rnn_size)
