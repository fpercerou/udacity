...
def test_get_batches(get_batches):
    with tf.Graph().as_default():
        ...
        for x in range(batches.shape[2]):
            expected = np.array(range(x * 35, x * 35 + batches.shape[3]))
            assert np.array_equal(batches[0,0,x], expected),\
                '''
                Batches returned wrong contents.
                For example, input sequence {} in the first batch was:
                {}
                but it should be:
                {}
                '''.format(x, batches[0,0,x], expected)
