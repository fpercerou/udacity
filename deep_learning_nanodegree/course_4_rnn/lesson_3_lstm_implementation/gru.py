...
class GRU(AbstractRNN):
    """
    Class implementing GRU algo with tensorflow
    """
    def __init__(self, **kwargs):
        ...
        _sentinelle: put there to prevent positional parameters.
                     Do not touch.
        """
        if kwargs.pop('cell_type', 'GRU') != 'GRU':
            raise ValueError("cell_type must be 'GRU'")
        super(GRU, self).__init__(cell_type='GRU', **kwargs)

    def build_cell(self):
        cell = tf.nn.rnn_cell.GRUCell(self.multi_rnn_size)
...
if __name__ == '__main__':
    ...
    model = GRU(batch_size=batch_size,
                 n_steps=n_steps,
